...
- **Cookie の限定** … リフレッシュトークンは `HttpOnly / Secure / SameSite=Strict / Path=/refresh` の Cookie でのみ受け渡しし、JavaScript からの窃取と他エンドポイントへの送信を防ぐ
- **CSRF 対策** … `/refresh` は CSRF 保護を有効にし、`login` レスポンス時点で `XSRF-TOKEN` クッキーを先行発行して初回リフレッシュの鶏卵問題を回避する
